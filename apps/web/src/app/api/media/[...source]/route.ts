...
// Same ceiling as /api/collections/[address]/preview: every distinct seed
// index is a fresh eth_call, so the space must be bounded.
...
export async function GET(
  ...
  if (kind === "token" && tokenId && /^\d+$/.test(tokenId)) {
    ...
    if (kind === "renderer") {
      ...
      if (Number(tokenId) > MAX_SEED_INDEX) return notFound()
