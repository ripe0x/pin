/**
 * A collection's artist roster from Attribution.artistsOf, cross-checked
 * against each artist's own Catalog claim (see getAttribution's doc comment
  ...
  return (
    <section className="py-5 border-b border-gray-100">
      ...
      <ul className="space-y-2">
        {entries.map((e) => (
          <li key={e.artist} className="flex items-center justify-between gap-2 text-[11px] font-mono">
            <a
              href={evmNowAddressUrl(e.artist, chainId)}
              target="_blank"
              rel="noopener noreferrer"
              className="truncate text-fg-muted hover:text-fg hover:opacity-70 transition-opacity"
            >
              {shortAddress(e.artist)}
            </a>
            {e.claimed ? (
