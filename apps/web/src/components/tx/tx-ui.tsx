...
/**
 * GateHook require() reason strings (contracts/src/collection/hooks/
 * GateHook.sol) — these are Solidity `Error(string)` reverts, so viem exposes
 * the string itself as `reason` on the decoded error (or literally inside
 * shortMessage/metaMessages when it can't ABI-decode).
 */
const HOOK_REVERT_COPY: Record<string, string> = {
  "SC: not allowlisted": "This wallet is not on the allowlist for this mint.",
  "SC: wallet cap": "This wallet has reached its per-wallet mint limit.",
}
...
export function formatWriteError(err: unknown, action: string): string {
  ...
  // Walk the whole cause chain once, collecting every decoded error name,
  // require() reason, and message-shaped string we see, so a known Collection
  // protocol revert or hook revert maps to plain copy before falling back to
  // the generic deepest-message walk below.
  const seen: string[] = []
  ...
  for (const [name, copy] of Object.entries(COLLECTION_ERROR_COPY)) {
    const nameBoundary = new RegExp(`\\b${name}\\b`)
    if (seen.some((s) => s === name || nameBoundary.test(s))) return copy
  }
  for (const [reason, copy] of Object.entries(HOOK_REVERT_COPY)) {
    if (seen.some((s) => s.includes(reason))) return copy
  }
