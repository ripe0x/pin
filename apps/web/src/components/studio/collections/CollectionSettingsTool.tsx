...
/**
 * Entry point for the Collection Settings studio tool: pick a collection, then
 * manage its owner/admin levers (renderer, permanence locks, supply, royalty,
 * cover, minters, creators). The "your collections" list is the indexed
 * SurfaceCreated table (owner-filtered SELECT, from the server page); the
 * paste-an-address input carries collections the indexer doesn't have
 * (fork/sepolia, indexing lag, post-deploy transfer).
 */
  ...
  return (
    <div className="space-y-6">
      <header className="space-y-1.5">
        <h2 className="text-xl font-semibold tracking-tight">Collection settings</h2>
        <p className="text-sm text-gray-500 leading-relaxed">
          Manage the collection contract itself: swap or lock the renderer, set
          or lock the supply cap, adjust the royalty, edit the cover, grant
          extension minters, and edit the attribution roster. Only the
          collection&apos;s owner or an admin can change them. Locks are
      ...
      {!collection ? (
        ...
        <div className="space-y-4">
          ...
          <CollectionSettingsPanel collection={collection} />
        </div>
      )}
    </div>
  )
