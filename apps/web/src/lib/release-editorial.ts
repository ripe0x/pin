...
function loadEditorial(): ReleaseEditorial[] {
  try {
    return parseEditorial()
  } catch (error) {
    // A bad content entry must not take the home page down; the venue
    // falls back to its non-programmed selection.
    console.error("[release-editorial] ignoring invalid releases.json:", error)
    return []
  }
}

export const releaseEditorial = loadEditorial()
