...
function groupKey(event: EnrichedActivityEvent): {
  ...
  if (
    ...
    // One open auction per token at a time, so (house, contract, tokenId)
    // identifies the auction without a dedicated id column. PND auctions
    // carry `house`; Foundation auctions don't, so they scope on the
    // contract pair alone.
    const scope = event.house ? event.house.toLowerCase() : "fnd"
    return {
      kind: "bid",
      key: `bid:${scope}:${event.tokenContract.toLowerCase()}:${event.tokenId}`,
    }
  }
  return null
}
