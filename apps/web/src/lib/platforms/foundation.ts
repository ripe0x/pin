...
// Cancellable-listings discovery is two-sided:
//
//   - `public.fnd_cancellable_listings` — frozen full-history seed of
//     every FND seller's open listings (Foundation stopped accepting new
//     listings in late 2025, so this set only shrinks). Regenerated by
//     scripts/scan-fnd-active-auctions.mjs; see migration 022. This
//     replaced a bundled-JSON seed built from the v1 lazy-index snapshot,
//     which had only ~8% of live auctions — sellers outside PND's orbit
//     (like an FND-collection artist not in known_artists) were invisible.
//   - `${schema}.fnd_auctions` / `fnd_buy_nows` — live Ponder rows from
//     FND_START_BLOCK (~Oct 2025) onward, in case FND ever accepts a
//     listing again.
//
// Both are just the discovery side ("who *could* still have something
// cancellable"); the on-chain multicall below is the verification side
// ("what they actually have right now"), so stale rows are harmless.
...
export const foundationAdapter: PlatformAdapter = {
  ...
  async getCancellableListingsForSeller(seller: Address): Promise<SellerListings | null> {
    ...
    // in both.
    const rows = (await sql.unsafe(
      `SELECT kind, auction_id, contract, token_id
       FROM fnd_cancellable_listings
       WHERE seller = $1
       UNION ALL
       SELECT 'auction', auction_id::text, lower(nft_contract), token_id::text
       FROM ${schema}.fnd_auctions
       WHERE lower(seller) = $1 AND status = 'active'
       UNION ALL
       SELECT 'buyNow', NULL, lower(nft_contract), token_id::text
    ...
    const candAuctions: Array<{ auctionId: string; contract: string; tokenId: string }> = []
    const candBuyNows: Array<{ contract: string; tokenId: string }> = []
    const seenAuctions = new Set<string>()
    const seenBuyNows = new Set<string>()
    for (const r of rows) {
      if (r.kind === "auction" && r.auction_id) {
        if (seenAuctions.has(r.auction_id)) continue
        seenAuctions.add(r.auction_id)
        candAuctions.push({
          auctionId: r.auction_id,
          contract: r.contract,
          tokenId: r.token_id,
        })
      } else if (r.kind === "buyNow") {
        const key = `${r.contract}-${r.token_id}`
        if (seenBuyNows.has(key)) continue
        seenBuyNows.add(key)
        candBuyNows.push({ contract: r.contract, tokenId: r.token_id })
      }
    }
    ...
    for (let i = 0; i < candAuctions.length; i++) {
      const r = results[i]
      ...
      auctions.push({
        id: candAuctions[i].auctionId,
        platform: "foundation",
        auctionId: candAuctions[i].auctionId,
        nftContract: candAuctions[i].contract,
        tokenId: candAuctions[i].tokenId,
    ...
    for (let i = 0; i < candBuyNows.length; i++) {
      const r = results[candAuctions.length + i]
      if (r.status !== "success") continue
      const b = r.result as { seller: Address; price: bigint }
      if (b.seller.toLowerCase() !== lower) continue
      buyNows.push({
        id: `${candBuyNows[i].contract}-${candBuyNows[i].tokenId}`,
        platform: "foundation",
        nftContract: candBuyNows[i].contract,
        tokenId: candBuyNows[i].tokenId,
