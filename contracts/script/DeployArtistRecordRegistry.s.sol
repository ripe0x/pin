...
/// @notice Deploy ArtistRecordRegistry via the canonical CREATE2
...
contract DeployArtistRecordRegistryScript is Script {
    ...
    /// @dev Salt chosen for the ArtistRecordRegistry deploy. Combined
    ///      with the contract's creationCode and the deployer proxy's
    ///      address, this fixes the registry's address across chains.
    ///      Salt is `keccak256("ArtistRecordRegistry")` so a future
    ///      reader can recompute it from the contract name alone.
    bytes32 internal constant SALT =
        keccak256("ArtistRecordRegistry");

    function run() external {
        bytes memory bytecode = type(ArtistRecordRegistry).creationCode;
        ...
        console2.log("ArtistRecordRegistry deployed at:", predicted);
    }
}
