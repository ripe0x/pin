...
import {ISurfaceV2} from "../interfaces/ISurfaceV2.sol";
...
struct FixedPriceMinterV2InitParams {
    address collection;
...
/// @notice Canonical fixed-price/referral minter for a SurfaceV2 collection:
///         one EIP-1167 clone per collection, calling
///         `ISurfaceV2(collection).mintTo`.
///
///         Holds proceeds by pull payment (recipients withdraw); config
///         authority is borrowed from the collection (owner or admin). Every
///         mint through this contract is paid exactly `price * quantity`:
///         price 0 is legal config but not a free-mint special case, and
///         there is no owner-mint path and no overpay-refund accrual. A free
///         distribution requires granting a separate minter on the
///         collection.
///
...
contract FixedPriceMinterV2 is Initializable, ReentrancyGuardUpgradeable, IMinter {
    ...
    ///         Initialized to 0; collection owner/admin settable via
    ///         setReferralShareBps, capped at MAX_REFERRAL_SHARE_BPS. Not a
    ///         protocol fee. A mint with no referrer accrues the entire
    ///         amount to payoutRecipient.
    ...
    // recipient cannot block minting.
    ...
    function initialize(FixedPriceMinterV2InitParams calldata p) external initializer {
        ...
        emit MinterConfigured(
            p.collection, p.price, p.mintStart, p.mintEnd, p.payoutRecipient, p.maxMints, p.allowlistRoot, p.walletCap
        );
    }
    ...
    ///      misroute the Sold event's payer field. Reads msg.value directly:
    ///      an internal call preserves it.
        ...
        internal
    {
        ...
        // Fixed price only: payment must equal price * quantity exactly.
