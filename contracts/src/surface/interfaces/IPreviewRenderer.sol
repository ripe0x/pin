...
///         (see docs/injection-convention.md) so the work's code can
///         distinguish an exploratory render from a canonical token render.
///         Preview metadata carries no provenance attributes: a preview is
///         not a token.
interface IPreviewRenderer {
    ...
    ///         output (name marked as a preview, animation_url built from
    ///         `seed`, seed attribute only).
