...
contract MockPriceStrategy is IPriceStrategy {
    ...
    // guarantee, not just a convention. See SurfaceSecurity.t.sol for the
...
///      etc.) in tests.
contract MockMinter {
    function callMintTo(ISurface collection, address to, address referrer, bytes calldata hookData)
        external
...
contract RevertingPayee {
    ...
    function pull(ISurface collection) external {
...
contract ReenteringWithdrawer {
    ...
    function arm(ISurface target_) external {
        target = target_;
        armed = true;
    }
