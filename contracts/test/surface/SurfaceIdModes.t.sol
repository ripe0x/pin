...
contract SurfaceIdModesTest is SurfaceBase {
    ...
    function test_sequential_hasNoMintToId() public {
        ...
        (bool ok,) = address(c).call(abi.encodeWithSignature("mintToId(address,uint256)", collector, uint256(1)));
    ...
    function test_pooled_burnThenRemintSameId_freshSeedAndIndex() public {
        ...
        emit ISurfaceCore.Minted(address(minter), collector, 5, 1, 0);
        minter.callMintToId(IPooledSurface(address(c)), collector, 5);
        ...
        // mint order advances to 1 (never reused).
        vm.expectEmit(true, true, false, true, address(c));
        emit ISurfaceCore.Minted(address(minter), stranger, 5, 1, 1);
        minter.callMintToId(IPooledSurface(address(c)), stranger, 5);
    ...
    function test_pooled_liveIdCannotBeMintedOver() public {
        ...
        minter.callMintToId(IPooledSurface(address(c)), stranger, 1);
    }
    ...
    function test_pooled_id0IsMintable() public {
        ...
        c.setMinter(address(minter), true);
        vm.expectEmit(true, true, false, true, address(c));
        emit ISurfaceCore.Minted(address(minter), collector, 0, 1, 0);
        minter.callMintToId(IPooledSurface(address(c)), collector, 0);
    ...
    function test_sequential_idsNeverRecycleAfterBurn() public {
        Surface c = _collection(_freeConfig());
        _mintTo(c, collector, 3); // ids 1,2,3

        vm.prank(collector);
        c.burn(2);
        assertEq(c.totalSupply(), 2);

        _mintTo(c, collector, 1); // next id is 4, NOT the freed id 2
    ...
    function test_sequential_capBoundsMintsEver_burnDoesNotFreeSlots() public {
        ...
        _mintTo(c, collector, 2); // ids 1,2 -> mintedEver == cap
        vm.prank(collector);
        ...
        c.mintTo(collector, 1); // cap bounds EVER minted, burn does not free a slot
    ...
    function test_pooled_capBoundsLiveSupply_notMintsEver() public {
        ...
        minter.callMintToId(IPooledSurface(address(c)), collector, 3);
        minter.callMintToId(IPooledSurface(address(c)), collector, 4);
        assertEq(c.totalSupply(), 2);
    ...
    function test_pooled_zeroCapIsOpenSupply() public {
        ...
        for (uint256 i = 0; i < 10; i++) {
            minter.callMintToId(IPooledSurface(address(c)), collector, i);
        }
