...
import {MockPriceStrategy, RecordingHook} from "./mocks/CollectionMocks.sol";
...
contract CollectionMintForTest is CollectionBase {
    ...
    function test_mintFor_recipientOwns_payerPays() public {
        ...
        emit ICollectionCore.Minted(recipient, referrer, 1, 1, 0, CollectionStatus.Open);
        vm.prank(payer);
    ...
    function test_mintFor_exactPaymentRequired() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(ICollectionCore.WrongPayment.selector, 1 ether, 2 ether));
        vm.prank(payer);
