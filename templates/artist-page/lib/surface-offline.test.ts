...
describe("standalone Surface release path", () => {
  ...
  it("keeps PND hostnames out of the release-critical template modules", async () => {
    ...
    expect(await readFile(resolve(root, "components/CollectionTokenGrid.tsx"), "utf8"))
      .not.toMatch(/pnd\.ripe\.wtf|art-pin\.netlify\.app|from ["']@\/lib\/(metadata|collection-onchain)/i)
    const grid = await readFile(resolve(root, "components/CollectionTokenGrid.tsx"), "utf8")
    expect(grid).toMatch(/serializeTokenReads/)
    expect(grid).not.toMatch(/Promise\.all\([^)]*readToken/s)
  })

  it("keeps the artist release on the shared provider and React boundary", async () => {
    const source = await readFile(resolve(import.meta.dirname, "../components/CollectionMintCard.tsx"), "utf8")
    expect(source).toMatch(/@pin\/surface-react/)
    expect(source).toMatch(/createDirectChainSurfaceProvider/)
    expect(source).toMatch(/releaseAvailability/)
    expect(source).toMatch(/provider\.quoteMint/)
    expect(source).toMatch(/provider\.prepareMint/)
    expect(source).not.toMatch(/useReadContract/)
    expect(source).not.toMatch(/prepareFixedPriceMint/)
  })
})
