...
Implementation `0x88b48793f38EF7370F2e7BC12E2f73DC565C117F` · [view on evm.now](https://evm.now/address/0x88b48793f38EF7370F2e7BC12E2f73DC565C117F?chainId=1) · Ethereum mainnet

> Deployed per owner as an EIP-1167 clone of the implementation above through the [V2 factory](/docs/auctions/contracts/auction-house-v2-factory). A house has no canonical address of its own; substitute your house address for `<AUCTION_HOUSE_V2_ADDRESS>` in the examples below. This is a separate deployment from [SovereignAuctionHouse](/docs/auctions/contracts/sovereign-auction-house); the two coexist and neither depends on the other.
...
```solidity
function createAuction(uint256 tokenId, address tokenContract, uint256 duration, uint256 reservePrice, uint64 listingExpiry_) external returns (uint256)
```
...
```solidity
function bulkCreateAuctions(address tokenContract, uint256[] tokenIds, uint256 reservePrice, uint256 duration, uint64 listingExpiry_) external returns (uint256[] auctionIds)
```
...
```solidity
function create1155Auction(uint256 tokenId, address tokenContract, uint256 quantity, uint256 duration, uint256 reservePrice, uint64 listingExpiry_) external returns (uint256 auctionId)
```
...
```solidity
function recoverStuckERC1155(address tokenContract, uint256 tokenId, uint256 quantity, address to) external
```
...
```solidity
function auctions(uint256) external view returns (uint256 tokenId, address tokenContract, uint64 firstBidTime, uint256 amount, uint256 reservePrice, address tokenOwner, address payable fundsRecipient, uint64 endTime, address payable bidder, uint64 duration, uint256 quantity, ISovereignAuctionHouseV2.TokenStandard standard)
```
...
```solidity
function deliveryDeferredAt(uint256) external view returns (uint64)
```
...
```solidity
function getAuction(uint256 auctionId) external view returns (ISovereignAuctionHouseV2.Auction)
```
...
```solidity
function listingExpiry(uint256) external view returns (uint64)
```
...
```solidity
function PENDING_DELIVERY_TIMEOUT() external view returns (uint64)
```
...
```solidity
function pendingDelivery(uint256) external view returns (bool)
```
...
```solidity
function pendingReturn(uint256) external view returns (bool)
```
