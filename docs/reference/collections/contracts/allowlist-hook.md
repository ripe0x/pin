...
> Not yet deployed to Ethereum mainnet. The address lands here at launch; until then, examples use an `<ALLOWLIST_HOOK_ADDRESS>` placeholder.
...
```solidity
function beforeMint(address minter, uint256, uint256, address, bytes hookData) external view returns (bytes4)
```
