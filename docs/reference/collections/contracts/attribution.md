...
> Not yet deployed to Ethereum mainnet. The address lands here at launch; until then, examples use an `<ATTRIBUTION_ADDRESS>` placeholder.
...
```solidity
function setArtists(address collection, address[] artists) external
```
...
```solidity
function artistsOf(address collection) external view returns (address[])
```
...
```bash
cast call <ATTRIBUTION_ADDRESS> "artistsOf(address)(address[])" <COLLECTION_ADDRESS> \
...
```solidity
function artistsSlice(address collection, uint256 start, uint256 count) external view returns (address[])
```
