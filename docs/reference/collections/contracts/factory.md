...
```solidity
function createSurface(string name, string symbol, address owner, SurfaceConfig cfg, address[] initialMinters, address[] creators) external returns (address collection)
```
