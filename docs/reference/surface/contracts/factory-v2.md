...
> Not yet deployed to Ethereum mainnet. The address lands here at launch; until then, examples use an `<SURFACE_FACTORY_V2_ADDRESS>` placeholder.
...
```solidity
function createSurface(string name, string symbol, address owner, SurfaceConfig cfg, SaleConfig sale, address[] creators, address seedSource) external returns (address collection, address minter)
```
...
```solidity
function createSurfaceCustom(string name, string symbol, address owner, SurfaceConfig cfg, address[] initialMinters, address primaryMinter, address[] creators, address seedSource) external returns (address collection)
```
...
```bash
cast call <SURFACE_FACTORY_V2_ADDRESS> "catalog()(address)" \
